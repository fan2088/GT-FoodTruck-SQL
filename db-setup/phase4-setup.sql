...
register_main: BEGIN
    ...
    UPDATE err_msg SET err = 0;
    
    IF i_username IN (NULL, '')
        OR i_firstname IN (NULL, '')
        OR i_lastname IN (NULL, '')
        OR i_password IN (NULL, '') OR LENGTH(i_password) < 8
    THEN
        UPDATE err_msg SET err = 4;
        LEAVE register_main;
    END IF;
    
    IF EXISTS (SELECT username from `User` WHERE username = i_username)
    THEN
        UPDATE err_msg SET err = 1;
        LEAVE register_main;
    END IF;
    IF EXISTS (SELECT username from `User` WHERE (firstName, lastName) = (i_firstname, i_lastname))
    THEN
        UPDATE err_msg SET err = 2;
        LEAVE register_main;
    END IF;
    INSERT INTO `User` (username, `password`, firstName, lastName) VALUE (i_username, md5(i_password), i_firstName, i_lastName);
    
    IF i_balance > 0
    THEN
        INSERT INTO Customer (username, balance, stationName) VALUE (i_username, i_balance, NULL);
    END IF;
    
    IF i_email <> "" AND i_email IS NOT NULL
    THEN
        IF EXISTS (SELECT username from Employee WHERE email = i_email)
        THEN
            DELETE FROM `User` WHERE username = i_username;
            DELETE FROM Customer WHERE username = i_username;
            UPDATE err_msg SET err = 3;
            LEAVE register_main;
        END IF;
        INSERT INTO Employee (username, email) VALUE (i_username, i_email);
    ...
    IF NOT EXISTS (SELECT username FROM Staff WHERE username = i_username UNION SELECT username FROM Manager WHERE username = i_username UNION SELECT username FROM `Admin` WHERE username = i_username)
    THEN
        DELETE FROM `Employee` WHERE username = i_username;
    END IF;
    IF NOT EXISTS (SELECT username FROM Customer WHERE username = i_username UNION SELECT username FROM Employee WHERE username = i_username)
    THEN
        DELETE FROM `User` WHERE username = i_username;
    END IF;
END //
...
SELECT FoodTruck.foodTruckName AS foodTruckName, Station.stationName as stationName, StationRemainingCapacity.remainingCapacity as capacity, COUNT(DISTINCT username) as staffCount, COUNT(DISTINCT foodName) as menUItemCount
    FROM FoodTruck
    INNER JOIN Station
    ...
    INNER JOIN (
        select Station.stationName, Station.capacity - COUNT(distinct foodTruckName) as remainingCapacity from FoodTruck natural join Station group by stationName
    ) AS StationRemainingCapacity ON StationRemainingCapacity.stationName = Station.stationName
    WHERE
    ...
    (i_stationName = Station.stationName OR i_stationName = "" OR i_stationName IS NULL) AND
    ((i_hasRemainingCapacity = TRUE AND StationRemainingCapacity.remainingCapacity>0) OR (i_hasRemainingCapacity = FALSE))
...
mn_create_foodTruck_add_station_main: BEGIN

SET @stationCapacity = (SELECT capacity FROM Station WHERE stationName = i_stationName);
SET @stationNumFoodTrucks = (SELECT COUNT(*) FROM FoodTruck WHERE stationName = i_stationName);
SET @remainingCapacity = (@stationCapacity - @stationNumFoodTrucks);
IF @remainingCapacity <= 0
THEN
    LEAVE mn_create_foodTruck_add_station_main;
END IF;
...
mn_create_foodTruck_add_staff_main: BEGIN

SET @staffFoodTruck = (SELECT foodTruckName FROM Staff WHERE username = i_staffName);
IF @staffFoodTruck IS NOT NULL
THEN
    LEAVE mn_create_foodTruck_add_staff_main;
END IF;

UPDATE Staff SET foodTruckName = i_foodTruckName WHERE username = i_staffName;
...
mn_update_foodTruck_station_main: BEGIN
    ...
    IF @remainingCapacity <= 0
    THEN
        LEAVE mn_update_foodTruck_station_main;
    END IF;
...
BEGIN
    ...
    FROM
    (   SELECT FoodTruck.foodTruckName as foodTruckName, COUNT(DISTINCT (FilteredOrders.orderID)) as totalOrder, COALESCE(SUM(MenuItem.price * FilteredOrders.purchaseQuantity), 0) as totalRevenue, COUNT(DISTINCT customerUsername) as totalCustomer
        FROM FoodTruck
        LEFT JOIN MenuItem
        ON FoodTruck.foodTruckName = MenuItem.foodTruckName
        LEFT JOIN (
            SELECT Orders.orderID, OrderDetail.foodTruckName, OrderDetail.foodName, OrderDetail.purchaseQuantity, Orders.`date`, Orders.customerUsername FROM OrderDetail
            LEFT JOIN Orders
            ON OrderDetail.orderID = Orders.orderID
            WHERE (i_minDate IS NULL OR Orders.date >= i_minDate) AND (i_maxDate IS NULL OR Orders.date <= i_maxDate)
        ) as FilteredOrders
        ON (MenuItem.foodTruckName = FilteredOrders.foodTruckName AND MenuItem.foodName = FilteredOrders.foodName)
        WHERE 
        ...
        (i_stationName IS NULL OR FoodTruck.stationName LIKE CONCAT('%', i_stationName, '%') )
...
BEGIN
    ...
    -- place your code/solution here
    IF EXISTS (SELECT username FROM Customer WHERE username = i_customerUsername)
    THEN
        INSERT INTO cus_current_information_basic_result (stationName) VALUE ((SELECT stationName FROM Customer WHERE username = i_customerUsername));
        UPDATE cus_current_information_basic_result SET balance = ((SELECT balance FROM Customer WHERE username = i_customerUsername));
        UPDATE cus_current_information_basic_result SET buildingName = ((SELECT buildingName FROM Station WHERE stationName IN (SELECT stationName FROM Customer WHERE username = i_customerUsername)));
        UPDATE cus_current_information_basic_result SET `description` = ((SELECT `description` FROM Building WHERE buildingName IN (SELECT buildingName FROM Station WHERE stationName IN (SELECT stationName FROM Customer WHERE username = i_customerUsername))));
        UPDATE cus_current_information_basic_result SET tags = ((SELECT group_concat(tag) FROM BuildingTag WHERE buildingName IN (SELECT buildingName FROM Station WHERE stationName IN (SELECT stationName FROM Customer WHERE username = i_customerUsername)) GROUP BY buildingName));
    END IF;
...
BEGIN
    ...
    SELECT foodTruckName, managerName, foodNames 
    FROM (SELECT foodTruckName, (SELECT concat(firstName," ", lastName) from `User` where username = managerUserName)as managerName from FoodTruck)   AS FoodTruck 
    JOIN (SELECT GROUP_CONCAT(foodName) AS foodNames, foodTruckName AS name from MenuItem GROUP BY foodTruckName) AS foodInTruck
    on FoodTruck.foodTruckName = foodInTruck.name
    where foodTruckName in (SELECT foodTruckName from FoodTruck where stationName in (
    SELECT stationName from Customer where username = i_customerUsername));
...
IF i_customerUsername IN (SELECT userName FROM Customer)
    THEN
        INSERT INTO Orders (`date`, customerUsername) VALUE (i_date, i_customerUsername);
    END IF;
...
BEGIN
    ...
    -- place your code/solution here
    SET @i_customerUsername = (select customerUsername from Orders where orderID = i_orderId);
    SET @foodPrice = (select price from MenuItem where foodTruckName = i_foodTruckName AND foodName = i_foodName);
    SET @balance = (select balance from Customer where username = @i_customerUsername);
    IF i_purchaseQuantity > 0 AND @balance - @foodPrice * i_purchaseQuantity >= 0
    THEN
        ...
        UPDATE Customer SET balance = (@balance - @foodPrice * i_purchaseQuantity) WHERE username = @i_customerUsername;
    END IF;
...
BEGIN
    ...
    SELECT
        ...
        SUM(purchaseQuantity * price) AS orderTotal,
        GROUP_CONCAT(DISTINCT OrderDetail.foodName) as foodNames,
        SUM(purchaseQuantity) AS foodQuantity
    FROM Orders
    ...
    WHERE customerUsername = i_customerUsername GROUP BY Orders.orderID;

END //
DELIMITER ;

-- Query #extra 1: mn_get_available_station
DROP PROCEDURE IF EXISTS mn_get_available_station;
DELIMITER //
CREATE PROCEDURE mn_get_available_station(IN i_foodTruckName VARCHAR(50))
BEGIN

    DROP TABLE IF EXISTS mn_get_available_station_result;
    CREATE TABLE mn_get_available_station_result(stationName VARCHAR(50));
    
    INSERT INTO mn_get_available_station_result
    SELECT stationName FROM (
    SELECT Station.stationName as stationName, Station.capacity - COUNT(DISTINCT foodTruckName) AS remainingCapacity
    FROM Station LEFT JOIN FoodTruck ON Station.stationName = FoodTruck.stationName GROUP BY Station.stationName HAVING remainingCapacity > 0) as E
    UNION
    SELECT stationName FROM FoodTruck WHERE foodTruckName = i_foodTruckName;
